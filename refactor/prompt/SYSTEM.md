...
5. SDK 버전 규칙(강제): 아래 신버전 SDK 사용 규칙을 반드시 따르세요.
   - 패키지: `@google/genai`만 사용 (구버전 `@google/generative-ai` 금지)
   - 인스턴스화: `new GoogleGenAI({ apiKey: ... })` (객체 인자 필수)
   - 호출: `ai.models.generateContent({ ... })` (구 `getGenerativeModel` 금지)
   - 응답: `response.text` 속성 사용 (`response.text()` 금지)
