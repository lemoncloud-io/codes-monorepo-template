...
```txt
┌─────────────────────────────────────────────────────┐
│         Monorepo Root (npm node workspace)          │
└─────────────────────────────────────────────────────┘
                    │
        ┌───────────┴────────────┬─────────────┐
        │                        │             │
...
|------|------------|-----------|
| **1** | `apps/frontend/src/services/geminiService.ts` | **(Service 시그니처 리팩토링)** 함수에 인풋 파라미터를 단일 객체(예시 `$param: { name: string }`)로 패키징시켜서 API 호출용 `body`를 준비합니다. 그런다음 리턴타입을 맞춘다음 API 호출로 변경합니다. (예시 `apiClient.post<ReturnType>(path, $param).then(R => R?.data)`) `GoogleGenAI` 실행은 백엔드에서 하므로, 여기에서는 `import { GoogleGenAI } from "@google/genai"` 를 포함한 관련된 코드를 정리해줍니다. |
| **2** | `apps/frontend/src/services/geminiService.ts` | **(API 연동작업)** 매칭되는 API HTTP Endpoint는  **`/hello/:id/generate`** 형식으로 지정합니다. `:id` 값은 `geminiService.ts`의 메인 함수명을 camelCase -> dash-case로 변환하여 사용합니다. body에서 필요한 값을 추출해 `$param` 객체를 생성하고, 이를 API 요청에 대한 body로 사용합니다. (예시: `apiClient.post<ReturnType>('/hello/say-hello/generate', $param)`) |
