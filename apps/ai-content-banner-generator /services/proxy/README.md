...
`src/lib/proxy`는 HTTP 기반 agents API를 Gemini 호환 `ai.models.generateContent()` 표면으로 사용하는 어댑터를 관리합니다.
...
- 서버와 브라우저에서 동일하게 사용할 수 있도록 표준 `fetch`만 사용합니다.
...
응답 복원은 공통 함수 `$gemini.asGenerateContentResponse()`를 사용하며, `GeminiManager.byGenAIResponse()`도 같은 함수를 참조합니다.

```ts
import { HttpAbstractGenAI } from './src/lib/proxy';
...
응답은 agents API의 `GenAIResponse`를 `AbstractGenAIGenerateContentResponse`로 복원합니다.
...
- `mocks.ts`: `AgentAPIController.doPostGenerate()`를 fetch처럼 호출하기 위한 테스트 helper
...
`tools/wss-inline-data-probe.ts`는 실제 WebSocket connection id를 얻은 뒤 generate transport 루프를 확인하는 용도로 사용할 수 있습니다.

```bash
npx ts-node tools/wss-inline-data-probe.ts --generate --prompt "hello" --param system=/dump
```

이 프로브는 기본적으로 `http://localhost:8830/agents/!/generate`를 호출하고,
`transportId`에는 WSS 연결에서 받은 `wss.connectionId`를 사용합니다.

## Inline Image Dump Transport 예제

인라인 이미지를 포함한 요청이 `/dump` 결과로 WebSocket transport를 통해 돌아오는지는
`src/lib/proxy/proxy.spec.ts`의 다음 테스트를 기준으로 확인합니다.

```ts
const network = createNetwork();
const receiver = createProxyTransportReceiver<GenAIResponse<any>>(network, { timeoutMs: 1000 });

const controller = new (class extends AgentAPIController {
    protected sendGenerateTransport = async (transportId: string, payload: Record<string, any>) => {
        const sender = createJSONTransport<GenAIResponse<any>>(network);
        sender.send(payload);
        sender.detach();
...
- HTTP 응답은 ack이고, 최종 dump payload는 `sendGenerateTransport()`가 보낸 JSONTransport 패킷에서 조립됩니다.
...
브라우저 UI에서 transport 루프를 빠르게 점검할 때는 `BrowserWebSocketNetwork.dumpTest()`를 사용할 수 있습니다.
이 메서드는 WebSocket connection id 조회, `/agents/!/generate?transportId=...` 호출, JSONTransport 수신,
`/dump` 결과 검증을 한 번에 수행합니다.

```ts
const result = await BrowserWebSocketNetwork.dumpTest({
