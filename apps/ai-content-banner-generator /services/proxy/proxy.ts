...
export class HttpAbstractGenAI implements ProxyGenAI {
    ...
    private async waitTransport(task: () => Promise<unknown>): Promise<object> {
    ...
    private asAgentGenerateBody(params: ProxyGenAIGenerateContentParams): AgentGenerateBody {
    ...
    private withoutSystemInstruction(config?: ProxyGenAIConfig): ProxyGenAIConfig | undefined {
        if (!config) return undefined;
        const { systemInstruction, ...rest } = config;
        return Object.keys(rest).length > 0 ? rest : undefined;
    }

    private asPrompt(contents: ProxyGenAIContents): AgentGenerateBody['prompt'] {
        if (typeof contents === 'string') return contents;
        return { type: 'user', content: contents };
    }

    private isImageRequest(params: ProxyGenAIGenerateContentParams): boolean | undefined {
