...
/** options for `BrowserWebSocketNetwork.dumpTest()` */
...
/** result returned by `BrowserWebSocketNetwork.dumpTest()` for UI diagnostics */
...
const createDefaultWebSocket = (url: string): WebSocketCompatible => {
    ...
    if (!WebSocketCtor) throw new Error(`global WebSocket is not available - BrowserWebSocketNetwork.dumpTest`);
