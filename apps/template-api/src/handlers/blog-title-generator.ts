...
import { GeneratedContent } from '@shared/core';
...
async function generateBlogContent(keyword: string): Promise<GeneratedContent> {
  ...
  try {
    ...
    if (!keyword) {
      return {
        ...
        body: JSON.stringify({ message: 'Keyword is missing from request body' }),
      };
    }
