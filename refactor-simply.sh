...
LOG_FILE="${LOG_DIR}/refactor-simply.log"
...
echo "Starting steps for simply code refactoring..."

# 리팩토링 스크립트 실행
# ----------------------------------------------------------
echo "[1/2] Running refactor.ts for simply code..."
# npx ts-node refactor.ts 1 2>&1 | tee -a "$LOG_FILE"
npx ts-node refactor.ts simply 2>&1 | tee -a "$LOG_FILE"
