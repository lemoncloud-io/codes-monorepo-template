...
- Google AI Studio에서 생성된 App 코드를 우리 Company만의 **모노레포 백엔드 코드 패턴에 맞게 리팩토링**합니다.
...
|------|------------|-----------|
| **1** | `apps/backend/src/services/geminiService.ts` | **(Service 시그니처 리팩토링)** 유저 프롬프트에 `apps/backend/src/services/types.ts` 코드가 포함된 경우에만 상단에 **`./types`타입 import** 코드를 추가합니다. Aop의 메인 함수의 파라미터를 단일 객체(`$param: { name: string }` 예시)로 변경하고, 내부 참조를 `$param?.name`(예시) 형태로 수정합니다. |
